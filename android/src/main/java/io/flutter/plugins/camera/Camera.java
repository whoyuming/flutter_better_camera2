...
//      Log.d(TAG, "process " + mState);
      switch (mState) {
        case STATE_LOCKING: {
//          Log.d(TAG, "process STATE_LOCKING");
          Integer af = result.get(CaptureResult.CONTROL_AF_STATE);
          if (af == null) {
//            Log.d(TAG, "process STATE_LOCKING af == null");
            break;
          }
          if (af == CaptureResult.CONTROL_AF_STATE_FOCUSED_LOCKED
            ...
            if (ae == null ||
                    (CaptureResult.CONTROL_AF_STATE_PASSIVE_SCAN != af
...
//              Log.d(TAG, "process STATE_LOCKING 2");
              setState(STATE_LOCKED);
              onPrecaptureRequired();
            }
          } else {
//            Log.d(TAG, "process STATE_LOCKING af == " + af);
          }
          break;
        }
        case STATE_PRECAPTURE: {
//          Log.d(TAG, "process STATE_PRECAPTURE");
          Integer ae = result.get(CaptureResult.CONTROL_AE_STATE);
        ...
        case STATE_WAITING: {
//          Log.d(TAG, "process STATE_WAITING");
//          Integer ae = result.get(CaptureResult.CONTROL_AE_STATE);
//          if (ae == null || ae != CaptureResult.CONTROL_AE_STATE_PRECAPTURE) {
            setState(STATE_CAPTURING);
            onReady();
//          } else {
//            Log.d(TAG, "process STATE_WAITING : " + ae);
//          }
          break;
        }
      }
...
//      Log.d(TAG, "PrecatureRequired 1");
      setState(STATE_PRECAPTURE);
//      Log.d(TAG, "PrecatureRequired 2");
      try {
        mCaptureSession.capture(mPreviewRequestBuilder.build(), this, null);
//        Log.d(TAG, "PrecatureRequired 3");
        mPreviewRequestBuilder.set(CaptureRequest.CONTROL_AE_PRECAPTURE_TRIGGER,
                CaptureRequest.CONTROL_AE_PRECAPTURE_TRIGGER_IDLE);
//        Log.d(TAG, "PrecatureRequired 4");
      } catch (CameraAccessException e) {
...
//    Log.e(TAG, "takePicture 1");
    mCaptureCallback.setFilePath(filePath);
    mCaptureCallback.setResult(result);
//    Log.e(TAG, "takePicture 2");

//    if (mAutoFocus && zoomLevel == 1.0) {
    if (mAutoFocus) {
